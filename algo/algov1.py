...
class Algorythm():
    ...
    def __init__(self):
        ...
        self.teachers = pd.DataFrame(data=Teacher.objects.all().annotate(
            count_of_disciplines=Count('disciplines'),
        ).values())
        self.education_plan = pd.DataFrame(data=EducationPlan.objects.all().values())
        ...
        self.lecture_halls = pd.DataFrame(data=LectureHall.objects.all().annotate(
            building_code=F('building__code'),
        ).values())
    ...
    def _choose_lecture_halls(self, group, teacher, lessons, discipline_id):
        ...
        for day, lesson in lessons:
            ...
            if len(lessons_in_day):
                lh_filter &= (self.lecture_halls.building_code==self.lecture_halls.loc[self.lecture_halls.id==lessons_in_day.iloc[0], 'building_code'].iloc[0])
                ...
                while discipline.lessons_in_week != 0:
                    ...
                    for lecture_hall, day_of_week, lesson in lecture_halls:
                        if lecture_hall:
                            ...
                            discipline.lessons_in_week -= 1
                    exclude_lessons += lesson_times
                    lesson_times = self._choose_lesson_positions(teacher, group, discipline.lessons_in_week, exclude_lessons=exclude_lessons)
                    if len(lesson_times) != 0 and lesson_times[0][0] == 0:
                        disc = Discipline.objects.get(id=discipline.discipline_id)
                        print(f'Не получается распределить пары по {disc.title}'
                        f' в количистве {discipline.lessons_in_week} с преподавателем'
